...
export default function ContratistaDashboard({ user, onOpenReportEditor, onDirectPrint }: Props) {
  ...
  const loadContractorData = async () => {
    ...
    // Aplicar depuración automática a la lista local
    const { validReports } = await supabaseService.cleanupExpiredReports(localList, user.documentoIdentidad);

    const sorted = validReports.sort((a, b) => parseInt(b.informeNro || '0') - parseInt(a.informeNro || '0'));
    setReportsList(sorted);
    setNewInformeNro((Math.max(...sorted.map(r => parseInt(r.informeNro || '0')), 0) + 1).toString());

    // Detectar si hay informes aprobados en local para notificar
    const approved = sorted.filter(r => r.estado === 'Aprobado');
    if (approved.length > 0) {
      const unnotified = approved.filter(r => {
        const key = `notified_approved_${user.documentoIdentidad}_${r.informeNro}`;
        return sessionStorage.getItem(key) !== 'seen';
